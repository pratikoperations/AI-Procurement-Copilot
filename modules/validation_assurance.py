...
def run_validation_assurance(
    ...
    source_label = (
        ...
        if assumptions.get("data_source") == "Synthetic Demo"
        else "Uploaded supplier data — not independently verified"
    )
...
def safe_executive_text(eligibility: dict, approved_text: str, provisional_text: str | None = None) -> str:
    """Prevent polished final-award language when validation does not permit it."""
