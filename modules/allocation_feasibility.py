...
SUPPORTED_COMPLETE_ENUMERATION_SUPPLIERS = 10
...
def _logical_request_errors(request: MultiSupplierAllocationRequest) -> tuple[FeasibilityStatus | None, list[str]]:
    """Apply explicit primary-status precedence: INVALID_REQUEST before share conflicts."""
    invalid: list[str] = []
    share_conflicts: list[str] = []
    k = request.required_awardee_count
    if k < 1:
        invalid.append("required_awardee_count must be at least 1")
    if request.maximum_supplier_share_pct <= 0 or request.maximum_supplier_share_pct > 100:
        invalid.append("maximum_supplier_share_pct must be greater than 0 and no greater than 100")
    if request.capacity_utilization_ceiling_pct <= 0 or request.capacity_utilization_ceiling_pct > 100:
        invalid.append("capacity_utilization_ceiling_pct must be greater than 0 and no greater than 100")
    if request.minimum_risk_score > 100 or request.minimum_esg_score > 100:
        invalid.append("risk and ESG thresholds cannot exceed 100")
    if request.minimum_awarded_share_pct > request.maximum_supplier_share_pct:
        share_conflicts.append("minimum_awarded_share_pct cannot exceed maximum_supplier_share_pct")
    if request.minimum_continuity_share_pct > request.maximum_supplier_share_pct:
        share_conflicts.append("minimum_continuity_share_pct cannot exceed maximum_supplier_share_pct")
    if k * request.minimum_awarded_share_pct > 100 + 1e-9:
        share_conflicts.append("required_awardee_count multiplied by minimum_awarded_share_pct exceeds 100%")
    if k * request.maximum_supplier_share_pct < 100 - 1e-9:
        share_conflicts.append("required_awardee_count multiplied by maximum_supplier_share_pct cannot cover 100%")
    errors = sorted(set(invalid + share_conflicts))
    if invalid:
        return FeasibilityStatus.INVALID_REQUEST, errors
    if share_conflicts:
        return FeasibilityStatus.SHARE_CONSTRAINT_CONFLICT, errors
    return None, []
...
def evaluate_allocation_feasibility(
    ...
    *,
    ...
    """Evaluate whether an exactly-K supplier portfolio can cover demand."""
    ...
    if isinstance(max_combinations, bool) or not isinstance(max_combinations, int) or max_combinations < 1:
        return _result(request, FeasibilityStatus.INVALID_REQUEST, "Allocation request is not feasible.", blocking=("max_combinations must be a positive integer",))

    suppliers, invalid_capacity, invalid_records = _normalize_suppliers(supplier_records)
    if invalid_records:
        return _result(request, FeasibilityStatus.INVALID_REQUEST, "Supplier inputs contain invalid governed values.", blocking=invalid_records)
    ...
    if conflict:
        return _result(request, FeasibilityStatus.REQUIRED_EXCLUDED_CONFLICT, "A required supplier is also excluded.", blocking=("Required and excluded supplier conflict: " + ", ".join(conflict),))
    missing_required = sorted(required - supplier_ids)
    if missing_required:
        return _result(request, FeasibilityStatus.REQUIRED_SUPPLIER_MISSING, "One or more required suppliers are missing.", blocking=("Missing required suppliers: " + ", ".join(missing_required),))
    if len(required) > request.required_awardee_count:
        return _result(request, FeasibilityStatus.INVALID_REQUEST, "Required supplier count exceeds required awardee count.", blocking=("required_supplier_ids cannot contain more suppliers than required_awardee_count",))
    ...
    if ineligible_required:
        return _result(request, FeasibilityStatus.REQUIRED_SUPPLIER_INELIGIBLE, "One or more required suppliers do not satisfy eligibility thresholds.", eligible_count=len(commercially_eligible), blocking=("Required suppliers not eligible: " + ", ".join(ineligible_required),))
    if len(commercially_eligible) < request.required_awardee_count:
        return _result(request, FeasibilityStatus.INSUFFICIENT_ELIGIBLE_SUPPLIERS, "Eligible supplier count is below the required awardee count.", eligible_count=len(commercially_eligible), blocking=(f"Required {request.required_awardee_count} awardees but only {len(commercially_eligible)} suppliers are eligible",))

    relevant_invalid_capacity = [reason for reason in invalid_capacity if reason.split(":", 1)[0] in eligible_ids]
    evidence = []
    ...
    if relevant_invalid_capacity:
        return _result(request, FeasibilityStatus.MISSING_CAPACITY_EVIDENCE, "Eligible supplier capacity evidence is missing or invalid; no capacity was inferred.", eligible_count=len(commercially_eligible), feasible_supplier_count=sum(share + 1e-9 >= request.minimum_awarded_share_pct for share in maximum_shares.values()), evidence=evidence_tuple, maximum_shares=maximum_shares, blocking=relevant_invalid_capacity, binding=("supplier capacity evidence",))

    minimum = request.minimum_awarded_share_pct
    incapable_required = sorted(item for item in required if maximum_shares[item] + 1e-9 < minimum)
    if incapable_required:
        return _result(request, FeasibilityStatus.INSUFFICIENT_CAPACITY, "A required supplier cannot support the minimum awarded share.", eligible_count=len(commercially_eligible), feasible_supplier_count=sum(share + 1e-9 >= minimum for share in maximum_shares.values()), evidence=evidence_tuple, maximum_shares=maximum_shares, blocking=(f"Required suppliers below the {minimum:g}% minimum share: " + ", ".join(incapable_required),), binding=("required supplier", "minimum awarded share", "supplier capacity"))
    ...
    if len(capable) < request.required_awardee_count:
        return _result(request, FeasibilityStatus.INSUFFICIENT_CAPACITY, "Fewer than K eligible suppliers can support the minimum awarded share.", eligible_count=len(commercially_eligible), feasible_supplier_count=len(capable), evidence=evidence_tuple, maximum_shares=maximum_shares, blocking=(f"Only {len(capable)} suppliers can support the {minimum:g}% minimum share required for {request.required_awardee_count} awardees",), binding=("minimum awarded share", "supplier capacity"))
    ...
    evaluated = 0
    truncated = False
    complete_required = len(suppliers) <= SUPPORTED_COMPLETE_ENUMERATION_SUPPLIERS
    policy = "complete_deterministic_enumeration"
    for optional in combinations(available_optional, slots):
        if not complete_required and evaluated >= max_combinations:
            truncated = True
    ...
    if not feasible and truncated:
        return _result(request, FeasibilityStatus.ENUMERATION_LIMIT_REACHED, "Feasibility was not fully determined because the deterministic enumeration limit was reached.", eligible_count=len(commercially_eligible), feasible_supplier_count=len(capable), evidence=evidence_tuple, maximum_shares=maximum_shares, blocking=("Combination evaluation reached the disclosed deterministic bound before a feasible portfolio was found",), warnings=("Feasibility is indeterminate; do not treat this result as infeasible.", "Human procurement review is mandatory."), binding=("enumeration limit",), enumeration_policy=policy, combinations_evaluated=evaluated, truncated=True, decision_complete=False)

    if not feasible:
        total_capacity_share = sum(sorted(maximum_shares.values(), reverse=True)[: request.required_awardee_count])
        status = FeasibilityStatus.INSUFFICIENT_CAPACITY if total_capacity_share + 1e-9 < 100.0 else FeasibilityStatus.NO_FEASIBLE_K_SUPPLIER_PORTFOLIO
        return _result(request, status, "No feasible exactly-K supplier portfolio was found.", eligible_count=len(commercially_eligible), feasible_supplier_count=len(capable), evidence=evidence_tuple, maximum_shares=maximum_shares, blocking=(f"No exactly-{request.required_awardee_count} supplier portfolio satisfies all share, capacity and continuity constraints",), warnings=("Supplier capacity is supplied evidence and has not been independently verified.",), binding=("supplier capacity", "awardee count", "share constraints", "continuity supplier share"), enumeration_policy=policy, combinations_evaluated=evaluated)

    return _result(request, FeasibilityStatus.FEASIBLE, f"At least one feasible exactly-{request.required_awardee_count} supplier portfolio can cover 100% of demand.", eligible_count=len(commercially_eligible), feasible_supplier_count=len(capable), evidence=evidence_tuple, maximum_shares=maximum_shares, warnings=("Feasibility is decision support only; supplier capacity is not independently verified.", "Human procurement approval remains mandatory."), feasible_combinations=tuple(feasible), binding=("exact awardee count", "supplier capacity", "minimum and maximum share", "continuity supplier share"), enumeration_policy=policy, combinations_evaluated=evaluated, truncated=truncated)
