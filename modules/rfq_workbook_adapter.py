...
class CanonicalRecord:
    ...
    def valid_for_analysis(self) -> bool:
        """Backward-compatible alias for downstream review code."""
    ...
    for record in records:
        for field_name in positive:
            ...
            if value is not None and value <= 0:
                findings.append(Finding("Blocking", "POSITIVE_VALUE_REQUIRED", f"'{field_name}' must be greater than zero.", record.sheet, record.provenance.source_row_number, field_name))
        for field_name in non_negative:
            value = record.canonical_values.get(field_name)
            if value is not None and value < 0:
                findings.append(Finding("Blocking", "NEGATIVE_VALUE_REJECTED", f"'{field_name}' cannot be negative.", record.sheet, record.provenance.source_row_number, field_name))
...
def _validate_keys(records: Sequence[CanonicalRecord], findings: list[Finding]) -> None:
    ...
    for record in records:
        ...
        if prior is None:
            ...
            findings.append(Finding("Fatal", "CONTRADICTORY_DUPLICATE_KEY", f"Contradictory duplicate key {key}.", record.sheet, record.provenance.source_row_number))
        else:
            findings.append(Finding("Warning", "EXACT_DUPLICATE_ROW", f"Duplicate business payload for key {key}.", record.sheet, record.provenance.source_row_number))


def _validate_metadata(metadata_records: Sequence[CanonicalRecord], schema: Mapping[str, Any], findings: list[Finding]) -> Mapping[str, Any] | None:
    if len(metadata_records) > 1:
        findings.append(Finding("Fatal", "UPLOAD_METADATA_CARDINALITY_INVALID", "UPLOAD_METADATA may contain at most one non-empty data row.", "UPLOAD_METADATA"))
    ...
    row = metadata_records[0].provenance.source_row_number
    if str(metadata.get("SCHEMA_VERSION") or "") != expected_version:
        findings.append(Finding("Fatal", "SCHEMA_VERSION_UNSUPPORTED", f"SCHEMA_VERSION must equal '{expected_version}'.", "UPLOAD_METADATA", row, "SCHEMA_VERSION"))
    ...
    if upload_mode is not None and upload_modes and upload_mode not in upload_modes:
        findings.append(Finding("Fatal", "UPLOAD_MODE_INVALID", f"Unsupported upload mode '{upload_mode}'.", "UPLOAD_METADATA", row, "UPLOAD_MODE"))
    anonymization_values = set(constraints.get("anonymization_enum", ()))
    anonymization = metadata.get("ANONYMIZATION_STATUS")
    if anonymization is not None and anonymization_values and anonymization not in anonymization_values:
        findings.append(Finding("Blocking", "ANONYMIZATION_STATUS_INVALID", f"ANONYMIZATION_STATUS must be one of {sorted(anonymization_values)}.", "UPLOAD_METADATA", row, "ANONYMIZATION_STATUS"))
    ...
    if source_hash is None or re.fullmatch(hash_pattern, str(source_hash)) is None:
        findings.append(Finding("Blocking", "SOURCE_FILE_HASH_INVALID", "SOURCE_FILE_HASH_SHA256 must contain exactly 64 hexadecimal characters.", "UPLOAD_METADATA", row, "SOURCE_FILE_HASH_SHA256"))
    ...
    if base_currency is not None and re.fullmatch(currency_pattern, str(base_currency)) is None:
        findings.append(Finding("Blocking", "CURRENCY_FORMAT_INVALID", "BASE_CURRENCY must be a three-letter uppercase currency code.", "UPLOAD_METADATA", row, "BASE_CURRENCY"))
    return metadata
...
def _validate_row_currencies(records: Sequence[CanonicalRecord], schema: Mapping[str, Any], findings: list[Finding]) -> None:
    ...
    for record in records:
        ...
        if currency is not None and re.fullmatch(pattern, str(currency)) is None:
            findings.append(Finding("Blocking", "CURRENCY_FORMAT_INVALID", "CURRENCY must be a three-letter uppercase currency code.", record.sheet, record.provenance.source_row_number, "CURRENCY"))
...
def _record_row_valid(record: CanonicalRecord, findings: Sequence[Finding]) -> bool:
    ...
    for finding in findings:
        ...
        if finding.row_number == row_number:
            return False
        if finding.row_number is None and finding.code in SHEET_LEVEL_ROW_BLOCKING_CODES:
            return False
    return True
...
def _activate_latest_valid_versions(records: Sequence[CanonicalRecord], findings: list[Finding]) -> list[CanonicalRecord]:
    ...
    for key, group in groups.items():
        valid_rows = [item for item in group if item.row_valid and isinstance(item.canonical_values.get("QUOTATION_VERSION"), int)]
        if not valid_rows:
            result.extend(replace(item, active=False, eligible_for_analysis=False) for item in group)
            continue
        latest = max(item.canonical_values["QUOTATION_VERSION"] for item in valid_rows)
        winners = [item for item in valid_rows if item.canonical_values.get("QUOTATION_VERSION") == latest]
        if len(winners) != 1:
            findings.append(Finding("Blocking", "QUOTATION_VERSION_CONFLICT", f"Highest valid quotation version is ambiguous for group {key}.", "RFQ_QUOTES"))
            result.extend(replace(item, active=False, eligible_for_analysis=False) for item in group)
            continue
        winner = winners[0]
        result.extend(replace(item, active=item is winner, eligible_for_analysis=item is winner and item.row_valid) for item in group)
    return result


def _apply_event_eligibility(records: Sequence[CanonicalRecord], selected_event: str | None, selection_required: bool) -> list[CanonicalRecord]:
    if selection_required:
        return [replace(item, eligible_for_analysis=False) for item in records]
    if selected_event is None:
        return list(records)
    return [replace(item, eligible_for_analysis=item.eligible_for_analysis and str(item.canonical_values.get("SOURCING_EVENT_ID")) == selected_event) for item in records]


def _validate_no_valid_rfq(rfq_records: Sequence[CanonicalRecord], findings: list[Finding], *, selected_event: str | None) -> None:
    if not rfq_records:
        findings.append(Finding("Fatal", "RFQ_QUOTES_EMPTY", "RFQ_QUOTES must contain at least one non-empty quotation row.", "RFQ_QUOTES"))
        return
    if not any(item.active and item.eligible_for_analysis for item in rfq_records):
        code = "NO_VALID_SELECTED_EVENT_QUOTATIONS" if selected_event is not None else "NO_VALID_QUOTATION_RECORDS"
        message = f"No valid active quotation records remain for selected event '{selected_event}'." if selected_event is not None else "No valid active quotation records remain after adapter validation."
        findings.append(Finding("Fatal", code, message, "RFQ_QUOTES"))


def _validate_supplier_counts(rfq_records: Sequence[CanonicalRecord], findings: list[Finding]) -> None:
    suppliers_by_item: dict[tuple[Any, Any], set[Any]] = {}
    for item in rfq_records:
        if not item.active or not item.eligible_for_analysis:
            continue
    ...
    for item_key, suppliers in suppliers_by_item.items():
        if len({supplier for supplier in suppliers if supplier is not None}) < 2:
            findings.append(Finding("Blocking", "MINIMUM_SUPPLIER_COUNT_NOT_MET", f"RFQ item {item_key} has fewer than two valid suppliers.", "RFQ_QUOTES"))
...
def adapt_v13_workbook(
    ...
    *,
    ...
    rfq_records = _activate_latest_valid_versions(rfq_records, findings)
    event_ids = tuple(sorted({str(item.canonical_values["SOURCING_EVENT_ID"]) for item in rfq_records if item.canonical_values.get("SOURCING_EVENT_ID") is not None}))
    selection_required = len(event_ids) > 1 and selected_sourcing_event_id is None
    if selection_required:
        findings.append(Finding("Blocking", "SOURCING_EVENT_SELECTION_REQUIRED", "Multiple sourcing events require explicit selection.", "RFQ_QUOTES"))
    if selected_sourcing_event_id is not None and selected_sourcing_event_id not in event_ids:
        findings.append(Finding("Fatal", "SOURCING_EVENT_SELECTION_INVALID", f"Selected sourcing event '{selected_sourcing_event_id}' is not present.", "RFQ_QUOTES"))
    if selected_sourcing_event_id is not None:
        rfq_records = [item for item in rfq_records if str(item.canonical_values.get("SOURCING_EVENT_ID")) == selected_sourcing_event_id]
