...
def _history_match(
    quote: Any,
    eligible_history: tuple[EnrichedRecord, ...],
    *,
    approved_history_mappings: Mapping[str, str],
    manual_history_confirmations: set[tuple[str, str]],
) -> HistoricalMatch:
    ...
    if len(exact) == 1:
        return HistoricalMatch(True, "EXACT_MATERIAL_ID", exact[0].record.provenance.source_row_id, "Unique exact material match.")
    if len(exact) > 1:
        return HistoricalMatch(False, None, None, "Multiple exact material-history candidates require confirmation.")
    ...
    if len(descriptive) == 1:
        return HistoricalMatch(True, "MATERIAL_GROUP_DESCRIPTION", descriptive[0].record.provenance.source_row_id, "Unique material-group and normalized-description match.")
    if len(descriptive) > 1:
        return HistoricalMatch(False, None, None, "Multiple material-group and description candidates require confirmation.")
    ...
    if len(manual) == 1:
        return HistoricalMatch(True, "MANUAL_CONFIRMATION", manual[0].record.provenance.source_row_id, "Explicit manual history confirmation.")
    if len(manual) > 1:
        return HistoricalMatch(False, None, None, "Multiple manual confirmations are ambiguous.")
    return HistoricalMatch(False, None, None, "No deterministic eligible history match.")


def orchestrate_adapter_result(
    adapter_result: Any,
    *,
    ...
    resolved_date, date_source, date_findings = resolve_evaluation_date(
        adapter_result.upload_metadata, adapter_result.rfq_quotes, evaluation_date, today
    )
    currency = _comparison_currency(adapter_result, comparison_currency)
    quote_flags, history_flags, rule_findings = evaluate_conditional_rules(
        adapter_result, resolved_date, approved_free_text_row_ids=approved_free_text_row_ids
    )
    dynamic_findings: list[RuleFinding] = [*date_findings, *rule_findings]

    preliminary_history: list[EnrichedRecord] = []
    normalization_eligible_history_ids: set[str] = set()
    for record in adapter_result.po_history:
        ...
        eligible = history_flags.get(row_id, False) and not norm.blockers
        if eligible:
            normalization_eligible_history_ids.add(row_id)
        preliminary_history.append(EnrichedRecord(record, norm, eligible))

    current_history_ids, stale_findings = evaluate_history_staleness(
        adapter_result.po_history, normalization_eligible_history_ids, resolved_date
    )
    ...
    for record in adapter_result.rfq_quotes:
        norm = normalize_record(
            record.canonical_values,
            comparison_currency=currency,
            quantity_field="QUOTED_QUANTITY",
            uom_field="QUOTATION_UOM",
            price_field="BASE_UNIT_PRICE",
        )
        row_id = record.provenance.source_row_id
        ...
        evidence = quotation_coverage(
            record.canonical_values,
            norm.normalized_values,
            has_history_match=match.matched,
    ...
    normalization_blockers = [blocker for item in (*enriched_quotes, *enriched_history) for blocker in item.normalization.blockers]
    blockers = tuple(dict.fromkeys((*adapter_blockers, *orchestration_blockers, *normalization_blockers)))
