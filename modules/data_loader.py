...
def get_raw_material_demo_suppliers(commodity="PET Resin"):
    ...
    prices={"PET Resin":1.27,"Polyethylene":1.34,"Polypropylene":1.30,"Aluminium Foil":3.18,"Steel":1.04,"Copper":9.65}
...
def get_demo_data(category="Packaging Procurement",commodity="Corrugated Board",selected_structure: str|None=None):
    if category=="Raw Material Procurement": data=get_raw_material_demo_suppliers(commodity)
    ...
    else: data=get_demo_suppliers()
    data.attrs.update({"source_label":"Synthetic demonstration data","assumption_profile_version":"C2.0" if commodity=="Flexible Laminates" else ("C1.0" if commodity=="Kraft Paper" else "S1"),"category":category,"commodity":commodity})
    return data
...
def load_uploaded_rfq(uploaded_file):
    ...
    if raw_df.empty: raise RFQUploadError("The uploaded RFQ contains headers but no supplier rows. Add at least one supplier quotation and upload the file again.")
