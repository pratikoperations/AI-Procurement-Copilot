...
def normalize_steel_dependent_state(values: Mapping) -> dict:
    """Clear stale dependent values and validate every controlling Steel state."""
    ...
    if profile == "CR_COIL_COMMERCIAL":
        ...
        zinc = zinc if zinc > 0 else 0.08
        paint = paint if paint > 0 else 0.12
...
def render_steel_governed_dashboard(suppliers: pd.DataFrame, assumptions: Mapping) -> dict:
    """Render the complete isolated Steel decision, scenario, allocation, and export route."""
    ...
    decision_columns = ["Supplier", "technical_eligible", "normalized_usd_per_kg", "equivalent_inr_per_kg", "generic_risk_score", "steel_risk_score", "governed_total_score", "governed_rank", "eligibility_failure_reasons"]
    decision = scored[[column for column in decision_columns if column in scored.columns]].copy().rename(columns={
    ...
    return {"state": state, "summary": summary, "details": details, "manifest": manifest, "excel": excel_bytes, "json": json_bytes}
