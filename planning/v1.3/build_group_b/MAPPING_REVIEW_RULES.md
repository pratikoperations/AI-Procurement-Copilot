...
## Field-source classifications

Every canonical field must declare one source class:

- `SOURCE_MAPPED` — directly mapped from an approved source header.
- `USER_CONFIRMED` — supplied or confirmed by the user during mapping review.
- `DERIVED` — calculated from governed source fields with recorded transformation provenance.
- `SYSTEM_GENERATED` — generated by the application, such as stable source-row identifiers.
- `REFERENCE_ENRICHED` — supplied from an approved reference snapshot.

The mapping review must display the source class. A field classified as `DERIVED`, `SYSTEM_GENERATED` or `REFERENCE_ENRICHED` must not be presented as a direct SAP-export mapping.
