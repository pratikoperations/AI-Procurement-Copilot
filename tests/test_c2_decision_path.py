...
from modules.flexible_laminate_risk import ELIGIBILITY_THRESHOLDS, apply_flexible_laminate_risk_to_tco, assess_flexible_laminate_supplier
...
def _assumptions(structure="PET / PE"):
    return {"category":"Packaging Procurement","commodity":"Flexible Laminates","laminate_structure":structure,"annual_volume":500000,"raw_material_shock":0,"freight_shock":0,"demand_change":0,"fx_rate":83}
...
def test_each_eligibility_threshold_boundary_passes(field, bounds):
    ...
    record=_eligible_record(); record[field]=minimum if minimum is not None else maximum
...
def test_each_eligibility_threshold_breach_fails(field, bounds):
    minimum,maximum=bounds
    record=_eligible_record(); record[field]=(minimum-0.1) if minimum is not None else (maximum+0.1)
...
def test_invalid_risk_range_fails_validation():
    data=get_flexible_laminate_demo_suppliers("PET / PE"); data.loc[0,"Bond Strength Continuity Score"]=101
    result=validate_flexible_laminate_dataframe(data,"PET / PE")
    assert not result["is_valid"] and any("Bond Strength" in item for item in result["errors"])
...
def test_process_loss_affects_risk_and_tco():
    ...
    low_result=apply_flexible_laminate_risk_to_tco(low,500000); high_result=apply_flexible_laminate_risk_to_tco(high,500000)
...
def test_risk_quality_changes_ranking():
    ...
    data.loc[0,"Quoted Unit Price USD"]=2.10; data.loc[1,"Quoted Unit Price USD"]=2.10
...
def test_ineligible_supplier_excluded_from_standard_and_optimized_allocation():
    data=get_flexible_laminate_demo_suppliers("PET / PE"); data.loc[2,"Seal Integrity Continuity Score"]=60
    scored=enrich_supplier_scores(data,_assumptions())
...
def test_all_ineligible_returns_no_allocation_and_blocks_recommendation():
    data=get_flexible_laminate_demo_suppliers("PET / PE"); data["Application Approval Status"]="Not approved"
    scored=enrich_supplier_scores(data,_assumptions())
    ...
    assert optimized["allocation_df"].empty and "No technically eligible" in optimized["explanation"]
    result=validate_scored_output(scored)
    assert not result["is_valid"] and any("No technically eligible supplier" in item for item in result["errors"])
