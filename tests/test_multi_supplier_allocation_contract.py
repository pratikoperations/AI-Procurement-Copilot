...
def test_request_normalizes_supplier_identifiers_and_serializes_stably():
    value = request(
        required_supplier_ids=(" Vendor B ", "VENDOR A", "vendor a"),
        excluded_supplier_ids=(" Vendor F ",),
    )
...
def test_supplier_contract_supports_required_common_fields():
    supplier = SupplierAllocationInput(
        ...
        category_specific_eligibility_evidence={"plant_approved": True},
    )
    assert supplier.supplier_id == "vendor a"
    assert supplier.supplier_capacity == 800
    assert supplier.category_specific_eligibility_evidence["plant_approved"] is True


def test_supplier_mapping_accepts_existing_project_column_names():
    supplier = SupplierAllocationInput.from_mapping(
        {
            "Supplier": "Vendor A",
            "technical_eligible": True,
            "adjusted_tco_unit_usd": 1.2,
            "total_score": 85,
            "risk_score": 75,
            "performance_score": 80,
            "esg_score": 65,
            "Supplier Capacity": 900,
            "technical_ineligibility_reasons": "",
        }
    )
    assert supplier.supplier_id == "vendor a"
    assert supplier.supplier_capacity == 900
...
def test_feasibility_result_serialization_is_stable_and_human_review_is_mandatory():
    result = MultiSupplierFeasibilityResult(
        feasible=True,
        status_code=FeasibilityStatus.FEASIBLE,
        summary="Feasible",
        eligible_supplier_count=3,
        required_awardee_count=2,
        feasible_supplier_count=3,
        supplier_capacity_evidence=({"supplier_id": "vendor a"},),
        maximum_feasible_share_by_supplier={"vendor a": 70.0},
        blocking_reasons=(),
        warnings=("Human review required",),
        feasible_supplier_combinations=(("vendor a", "vendor b"),),
        binding_constraints=("capacity",),
    )
    assert result.human_review_required is True
    assert result.to_json() == result.to_json()
    assert json.loads(result.to_json())["status_code"] == "FEASIBLE"
