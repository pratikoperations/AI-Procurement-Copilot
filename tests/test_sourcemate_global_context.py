...
def test_all_streamlit_entry_points_use_one_shared_sourcemate_shell_contract():
    ...
    assert 'mount_global_sourcemate("Governed Calculation Explorer")' not in explorer_source
...
def test_widget_uses_shared_history_and_persistent_panel_without_fragile_render_guard():
...
def test_submit_updates_history_in_same_run_and_keeps_panel_open():
    source = Path("modules/sourcemate_conversation_ui.py").read_text(encoding="utf-8")
    ...
    assert "reset_global_mount_guard" not in source


def test_launcher_and_panel_controls_use_session_state_callbacks_without_starter_clutter():
    source = Path("modules/sourcemate_conversation_ui.py").read_text(encoding="utf-8")
